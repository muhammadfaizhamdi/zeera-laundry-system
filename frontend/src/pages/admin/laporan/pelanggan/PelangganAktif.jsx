...
export default function PelangganAktif() {
  ...
  // Mengamankan data dari API agar tak bisa memicu crash
  const { data = [], isLoading } = usePelangganData(activeFilter);
  const sortedData = [...data].sort((a, b) => (Number(b.total_transaksi) || 0) - (Number(a.total_transaksi) || 0));
  ...
  return (
    <div className="flex-1 flex flex-col gap-6 w-full h-full overflow-y-auto pr-2 custom-scrollbar">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white/90 backdrop-blur-xl rounded-[2rem] shadow-sm p-6 md:p-8 border border-slate-100 shrink-0 relative z-50">
        <div><h1 className="text-3xl font-extrabold text-slate-800 mb-1">Pelanggan Aktif</h1><p className="text-slate-500 font-medium">Daftar pelanggan berdasarkan intensitas transaksi.</p></div>
      ...
      <div className="flex-1 flex flex-col bg-white/90 backdrop-blur-xl border border-slate-100 rounded-[2rem] shadow-[0_30px_60px_rgba(8,112,184,0.08)] overflow-hidden min-h-0 p-6 md:p-8">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 min-h-0">
          <table className="w-full text-left border-collapse table-fixed">
            <thead className="sticky top-0 bg-white z-10 shadow-sm border-b border-slate-100"><tr className="text-slate-400 font-bold text-xs uppercase tracking-wider"><th className="py-4 pr-4 w-[25%]">PELANGGAN</th><th className="py-4 pr-4 w-[25%]">NO. WA</th><th className="py-4 pr-4 w-[25%]">TERAKHIR ORDER</th><th className="py-4 pr-4 w-[25%]">TOTAL TRANSAKSI</th></tr></thead>
            <tbody className="text-sm text-slate-700 divide-y divide-slate-50">
              {isLoading ? <tr><td colSpan="4" className="py-8 text-center text-slate-500 font-medium">Memuat data API...</td></tr> : sortedData.length === 0 ? <tr><td colSpan="4" className="py-8 text-center text-slate-500 font-medium">Tidak ada data pelanggan aktif.</td></tr> : sortedData.filter(d => {
