...
export default function TransaksiSelesai() {
  ...
  const dataSelesai = data.filter(p => p.status_pesanan === 'Selesai' || p.status_pesanan === 'Siap Ambil');

  const getBadgeHTML = (status) => {
    if(status === 'Siap Ambil') return <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium bg-slate-100 text-slate-700 border border-slate-200"><span className="w-1.5 h-1.5 rounded-full bg-slate-500"></span>Siap Ambil</span>;
    if(status === 'Selesai') return <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium bg-emerald-50 text-emerald-700 border border-emerald-100"><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"></path></svg>Selesai</span>;
    return null;
  };
  ...
  return (
    <div className="flex-1 flex flex-col gap-6 w-full h-full overflow-y-auto pr-2 custom-scrollbar">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white/90 backdrop-blur-xl rounded-[2rem] shadow-sm p-6 md:p-8 border border-slate-100 shrink-0 relative z-50">
        <div><h1 className="text-3xl font-extrabold text-slate-800 mb-1">Transaksi Selesai</h1><p className="text-slate-500 font-medium">Pesanan yang telah diselesaikan.</p></div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 min-h-0">
          <table className="w-full text-left border-collapse table-fixed">
            ...
            <tbody className="text-sm text-slate-700 divide-y divide-slate-50">
              {isLoading ? <tr><td colSpan="5" className="py-8 text-center text-slate-500">Memuat data API...</td></tr> : dataSelesai.length === 0 ? <tr><td colSpan="5" className="py-8 text-center text-slate-500">Tidak ada transaksi selesai.</td></tr> : dataSelesai.map(row => (
