...
export default function TransaksiBatal() {
  ...
  const dataBatal = data.filter(p => p.status_pesanan === 'Batal');
    ...
    <div className="flex-1 flex flex-col gap-6 w-full h-full overflow-y-auto pr-2 custom-scrollbar">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white/90 backdrop-blur-xl rounded-[2rem] shadow-sm p-6 md:p-8 border border-slate-100 shrink-0 relative z-50">
        ...
        <div className="flex items-center gap-3 relative">
          <button className="bg-[#f43f5e] text-white px-6 py-3 rounded-full font-bold shadow-lg shadow-rose-500/20 hover:bg-rose-600 hover:-translate-y-1 transition-all flex items-center gap-2 whitespace-nowrap"><span className="material-symbols-outlined text-[20px]">download</span> Export Laporan</button>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 min-h-0">
          <table className="w-full text-left border-collapse table-fixed">
            ...
            <tbody className="text-sm text-slate-700 divide-y divide-slate-50">
              {isLoading ? <tr><td colSpan="5" className="py-8 text-center text-slate-500">Memuat data API...</td></tr> : dataBatal.length === 0 ? <tr><td colSpan="5" className="py-8 text-center text-slate-500">Tidak ada transaksi batal.</td></tr> : dataBatal.map(row => (
                <tr key={row.id} className="hover:bg-slate-50/50 transition-colors"><td className="py-3.5 pr-4 text-slate-500 font-medium">{formatTgl(row.id_transaksi)}</td><td className="py-3.5 pr-4 font-bold text-slate-800 capitalize">{row.nama_pelanggan || 'Anonim'}</td><td className="py-3.5 pr-4 font-black text-[#0f766e]">Rp {(Number(row.total_harga)||0).toLocaleString('id-ID')}</td><td className="py-3.5 pr-4"><span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium bg-rose-50 text-rose-700 border border-rose-100"><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg>Batal</span></td><td className="py-3.5 pr-4"><button className="text-[#0F766E] font-bold hover:underline text-sm outline-none">Detail</button></td></tr>
              ))}
            </tbody>
          </table>
        </div>
